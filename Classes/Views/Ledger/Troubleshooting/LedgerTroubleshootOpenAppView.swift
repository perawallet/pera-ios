...
class LedgerTroubleshootOpenAppView: BaseView {
    ...
    private lazy var numberOneTextView: UITextView = {
        ...
        if let htmlData = "ledger-troubleshooting-open-app-first-html".localized.data(using: .unicode),
    ...
    private lazy var numberTwoTextView: UITextView = {
        ...
        textView.dataDetectorTypes = .link
        textView.textContainerInset = .zero
        if let htmlData = "ledger-troubleshooting-open-app-second-html".localized.data(using: .unicode),
            let attributedString = try? NSMutableAttributedString(
                data: htmlData,
                options: [.documentType: NSAttributedString.DocumentType.html],
                documentAttributes: nil) {
