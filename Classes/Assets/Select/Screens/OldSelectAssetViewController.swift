...
//
//   OldSelectAssetViewController.swift


import Foundation
import UIKit

class OldSelectAssetViewController: BaseViewController {

    private let layout = Layout<LayoutConstants>()

    weak var delegate: OldSelectAssetViewControllerDelegate?
...
extension OldSelectAssetViewController {
    ...
    private func initAccounts(with filter: FilterOption) -> [Account] {
        ...
        _ = allAccounts.removeAll { account -> Bool in
...
extension OldSelectAssetViewController: UICollectionViewDataSource {
...
extension OldSelectAssetViewController: UICollectionViewDelegateFlowLayout {
    func collectionView(
        _ collectionView: UICollectionView,
...
extension OldSelectAssetViewController {
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        case .none:
            if indexPath.item == 0 {
                delegate?.oldSelectAssetViewController(self, didSelectAlgosIn: account, forAction: transactionAction)
            } else {
                if let assetDetail = account.assetDetails[safe: indexPath.item - 1] {
                    delegate?.oldSelectAssetViewController(self, didSelect: assetDetail, in: account, forAction: transactionAction)
                }
            }
        case .algos:
            delegate?.oldSelectAssetViewController(self, didSelectAlgosIn: account, forAction: transactionAction)
...
protocol OldSelectAssetViewControllerDelegate: AnyObject {
