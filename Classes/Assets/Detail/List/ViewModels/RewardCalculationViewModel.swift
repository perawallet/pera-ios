...
class RewardCalculationViewModel {
    ...
    init(account: Account, calculatedRewards: Decimal, currency: Currency?) {
    ...
    private func setTotalAmount(from account: Account, and calculatedRewards: Decimal) {
        totalAmount = getTotalAmount(from: account, and: calculatedRewards).toAlgosStringForLabel
    }

    private func setCurrency(from account: Account, and currency: Currency?, and calculatedRewards: Decimal) {
        ...
        let currencyAmountForAccount = getTotalAmount(from: account, and: calculatedRewards) * priceDecimalValue
    ...
    private func setRewardAmount(from account: Account, and calculatedRewards: Decimal) {
