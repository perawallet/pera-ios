...
//   AssetAmountOutViewModel.swift

import Foundation
import MacaroonUIKit
import UIKit

struct AssetAmountOutViewModel: AssetAmountInputViewModel {
    private(set) var imageSource: ImageSource?
    private(set) var primaryValue: TextProvider?
    private(set) var isInputEditable = false
...
extension AssetAmountOutViewModel {
    ...
    mutating func bindDetail(
        ...
        guard let swapQuote,
              let amountOutUSDValue = swapQuote.amountOutUSDValue else {
            detail = "0.00".footnoteRegular()
            return
        }

        detail = amountOutUSDValue
