...
//   RekeyStandardToStandardAccountInstructionsScreen.swift

import Foundation
import MacaroonUIKit
import UIKit

final class RekeyStandardToStandardAccountInstructionsScreen: ScrollScreen {
    ...
    private let theme: RekeyStandardToStandardAccountInstructionsScreenTheme

    init(theme: RekeyStandardToStandardAccountInstructionsScreenTheme = .init()) {
        self.theme = theme
        super.init()
    }
...
extension RekeyStandardToStandardAccountInstructionsScreen {
...
extension RekeyStandardToStandardAccountInstructionsScreen {
    ...
    private func addFutureTransactionsSignInstruction() {
        let viewModel = RekeyStandardToStandardAccountFutureTransactionsSignInstructionViewModel(order: 1)
        addInstruction(viewModel)
    }

    private func addNoLongerAbleToSignInstruction() {
        let viewModel = RekeyStandardToStandardAccountNoLongerAbleToSignInstructionViewModel(order: 2)
        addInstruction(viewModel)
    }

    private func addNoConfigurationChangeInstruction() {
        let viewModel = RekeyStandardToStandardAccountNoConfigurationChangeInstructionViewModel(order: 3)
        addInstruction(viewModel)
    }
...
extension RekeyStandardToStandardAccountInstructionsScreen {
    ...
    private func bindBody() {
        let text =
            "rekey-standard-to-standard-account-instructions-body"
                .localized
                .bodyRegular()

        let hyperlink: ALGActiveType =
            .word("rekey-standard-to-standard-account-instructions-body-highlighted-text".localized)
