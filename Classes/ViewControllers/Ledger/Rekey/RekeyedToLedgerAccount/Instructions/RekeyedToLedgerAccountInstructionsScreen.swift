...
//   RekeyedToLedgerAccountInstructionsScreen.swift

import Foundation
import MacaroonUIKit
import UIKit

final class RekeyedToLedgerAccountInstructionsScreen: ScrollScreen {
    ...
    private let theme: RekeyedToStandardAccountInstructionsScreenTheme

    init(theme: RekeyedToStandardAccountInstructionsScreenTheme = .init()) {
        self.theme = theme
        super.init()
    }
...
extension RekeyedToLedgerAccountInstructionsScreen {
    ...
    private func addInstructions() {
        ...
        addContinueUnableToSignInstruction()
        addNoConfigurationChangeInstruction()
        addOpenBluetoothInstruction()
    }

    private func addFutureTransactionsSignInstruction() {
        let viewModel = RekeyedToLedgerAccountFutureTransactionsSignInstructionViewModel(order: 1)
        addInstruction(viewModel)
    }

    private func addContinueUnableToSignInstruction() {
        let viewModel = RekeyedToLedgerAccountContinueUnableToSignInstructionViewModel(order: 2)
        addInstruction(viewModel)
    }

    private func addNoConfigurationChangeInstruction() {
        let viewModel = RekeyedToLedgerAccountNoConfigurationChangeInstructionViewModel(order: 3)
        addInstruction(viewModel)
    }

    private func addOpenBluetoothInstruction() {
        let viewModel = RekeyedToLedgerAccountOpenBluetoothInstructionViewModel(order: 4)
        addInstruction(viewModel)
    }
...
extension RekeyedToLedgerAccountInstructionsScreen {
    ...
    private func bindBody() {
        let text =
            "rekeyed-to-ledger-account-instructions-body"
                .localized
                .bodyRegular()

        let hyperlink: ALGActiveType =
            .word("rekeyed-to-ledger-account-instructions-body-highlighted-text".localized)
