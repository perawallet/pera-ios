...
//   RekeyLedgerToLedgerAccountInstructionsScreen.swift

import Foundation
import MacaroonUIKit
import UIKit

final class RekeyLedgerToLedgerAccountInstructionsScreen: ScrollScreen {
    ...
    private let theme: RekeyLedgerToLedgerAccountInstructionsScreenTheme

    init(theme: RekeyLedgerToLedgerAccountInstructionsScreenTheme = .init()) {
        self.theme = theme
        super.init()
    }
...
extension RekeyLedgerToLedgerAccountInstructionsScreen {
...
extension RekeyLedgerToLedgerAccountInstructionsScreen {
    ...
    private func addInstructions() {
        ...
        addNoLongerConnectedInstruction()
        addNoConfigurationChangeInstruction()
        addOpenBluetoothInstruction()
    }

    private func addFutureTransactionsSignInstruction() {
        let viewModel = RekeyLedgerToLedgerAccountFutureTransactionsSignInstructionViewModel(order: 1)
        addInstruction(viewModel)
    }

    private func addNoLongerConnectedInstruction() {
        let viewModel = RekeyLedgerToLedgerAccountNoLongerConnectedInstructionViewModel(order: 2)
        addInstruction(viewModel)
    }

    private func addNoConfigurationChangeInstruction() {
        let viewModel = RekeyLedgerToLedgerAccountNoConfigurationChangeInstructionViewModel(order: 3)
        addInstruction(viewModel)
    }

    private func addOpenBluetoothInstruction() {
        let viewModel = RekeyLedgerToLedgerAccountOpenBluetoothInstructionViewModel(order: 4)
        addInstruction(viewModel)
    }
...
extension RekeyLedgerToLedgerAccountInstructionsScreen {
    ...
    private func bindBody() {
        let text =
            "rekey-ledger-to-ledger-account-instructions-body"
                .localized
                .bodyRegular()

        let hyperlink: ALGActiveType =
            .word("rekey-ledger-to-ledger-account-instructions-body-highlighted-text".localized)
