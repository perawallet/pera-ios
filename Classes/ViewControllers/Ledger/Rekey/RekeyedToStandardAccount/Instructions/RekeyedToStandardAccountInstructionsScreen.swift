...
//   RekeyedToStandardAccountInstructionsScreen.swift

import Foundation
import MacaroonUIKit
import UIKit

final class RekeyedToStandardAccountInstructionsScreen: ScrollScreen {
...
extension RekeyedToStandardAccountInstructionsScreen {
    ...
    private func addFutureTransactionsSignInstruction() {
        let viewModel = RekeyedToStandardAccountFutureTransactionsSignInstructionViewModel(order: 1)
        addInstruction(viewModel)
    }

    private func addContinueUnableToSignInstruction() {
        let viewModel = RekeyedToStandardAccountContinueUnableToSignInstructionViewModel(order: 2)
        addInstruction(viewModel)
    }

    private func addNoConfigurationChangeInstruction() {
        let viewModel = RekeyedToStandardAccountNoConfigurationChangeInstructionViewModel(order: 3)
        addInstruction(viewModel)
    }
...
extension RekeyedToStandardAccountInstructionsScreen {
    ...
    private func bindBody() {
        let text =
            "rekeyed-to-standard-account-instructions-body"
                .localized
                .bodyRegular()

        let hyperlink: ALGActiveType =
            .word("rekeyed-to-standard-account-instructions-body-highlighted-text".localized)
