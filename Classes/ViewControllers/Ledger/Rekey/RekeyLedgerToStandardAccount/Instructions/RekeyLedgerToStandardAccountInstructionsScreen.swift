...
//   RekeyLedgerToStandardAccountInstructionsScreen.swift

import Foundation
import MacaroonUIKit
import UIKit

final class RekeyLedgerToStandardAccountInstructionsScreen: ScrollScreen {
    ...
    private let theme: RekeyLedgerToStandardAccountInstructionsScreenTheme

    init(theme: RekeyLedgerToStandardAccountInstructionsScreenTheme = .init()) {
        self.theme = theme
        super.init()
    }
...
extension RekeyLedgerToStandardAccountInstructionsScreen {
...
extension RekeyLedgerToStandardAccountInstructionsScreen {
    ...
    private func addInstructions() {
        ...
        addNoLongerRequiredToSignInstruction()
        addNoConfigurationChangeInstruction()
        addOpenBluetoothInstruction()
    }

    private func addFutureTransactionsSignInstruction() {
        let viewModel = RekeyLedgerToStandardAccountFutureTransactionsSignInstructionViewModel(order: 1)
        addInstruction(viewModel)
    }

    private func addNoLongerRequiredToSignInstruction() {
        let viewModel = RekeyLedgerToStandardAccountNoLongerRequiredToSignInstructionViewModel(order: 2)
        addInstruction(viewModel)
    }

    private func addNoConfigurationChangeInstruction() {
        let viewModel = RekeyLedgerToStandardAccountNoConfigurationChangeInstructionViewModel(order: 3)
        addInstruction(viewModel)
    }

    private func addOpenBluetoothInstruction() {
        let viewModel = RekeyLedgerToStandardAccountOpenBluetoothInstructionViewModel(order: 4)
        addInstruction(viewModel)
    }
...
extension RekeyLedgerToStandardAccountInstructionsScreen {
    ...
    private func bindBody() {
        let text =
            "rekey-ledger-to-standard-account-instructions-body"
                .localized
                .bodyRegular()

        let hyperlink: ALGActiveType =
            .word("rekey-ledger-to-standard-account-instructions-body-highlighted-text".localized)
