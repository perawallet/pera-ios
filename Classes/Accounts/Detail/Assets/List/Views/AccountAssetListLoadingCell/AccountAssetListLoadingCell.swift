...
//   AccountAssetListLoadingCell.swift

import Foundation
import MacaroonUIKit
import UIKit

final class AccountAssetListLoadingCell: CollectionCell<AccountAssetListLoadingView> {
    override class var contextPaddings: LayoutPaddings {
        return (0, 24, 0, 24)
    }

    static let theme = AccountAssetListLoadingViewTheme()

    override func getContextView() -> AccountAssetListLoadingView {
        return AccountAssetListLoadingView(Self.theme)
    }
}

extension AccountAssetListLoadingCell {
    static func calculatePreferredSize(
        for theme: AccountAssetListLoadingViewTheme,
