...
final class DiscoverDeeplinksTests: XCTestCase {
    ...
    func testDiscoverDeeplink() {
        ...
        case .discover(let path):
            XCTAssertNil(path)
            let url = DiscoverURLGenerator.generateURL(destination: .home, theme: .unspecified, session: nil, enableDiscoverV5: false, network: .testnet)
    ...
    func testDiscoverDeeplinkWithBrowserPath() {
        ...
        case .discover(let path):
            ...
            let url = DiscoverURLGenerator.generateURL(path: path!, theme: .unspecified, session: nil, enableDiscoverV5: false, network: .testnet)
            XCTAssertEqual(expectedURL.extractBaseURL(), url?.extractBaseURL())
