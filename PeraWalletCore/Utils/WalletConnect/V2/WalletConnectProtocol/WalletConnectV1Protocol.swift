...
public final class WalletConnectV1Protocol:
    WalletConnectProtocol,
    ServerDelegate {
    var eventHandler: ((WalletConnectV1Event) -> Void)?

    public private(set) var sessionValidator:  WalletConnectSessionValidator = WalletConnectV1SessionValidator()
...
extension WalletConnectV1Protocol {
    ...
    private func registerToWCRequests() {
        ...
        wcRequestHandler.delegate = CoreAppConfiguration.shared?.wcDelegate
...
extension WalletConnectV1Protocol {
    ...
    public func disconnectFromSession(_ session: WCSession) {
        do {
...
extension WalletConnectV1Protocol {
    ...
    public var allWalletConnectSessions: [WCSession] {
        sessionSource.allWalletConnectSessions
    }

    public func getWalletConnectSession(for topic: WalletConnectTopic) -> WCSession? {
...
extension WalletConnectV1Protocol {
    ...
    public func clearExpiredSessionsIfNeeded() {
...
extension WalletConnectV1Protocol {
    public func isConnected(by url: WCURL) -> Bool {
        return walletConnectServer.isConnected(by: url)
    }

    public func register(_ handler: WalletConnectRequestHandler) {
    ...
    public func reconnect(to session: WalletConnectSession) throws {
        try walletConnectServer.reconnect(to: session)
    }

    public func disconnect(from session: WalletConnectSession) throws {
        try walletConnectServer.disconnect(from: session)
    }
    
    public func update(session: WalletConnectSession, with newWalletInfo: WalletConnectSessionWalletInfo) throws {
...
public typealias WalletConnectSession = WalletConnectSwift.Session
public typealias WalletConnectURL = WCURL
public typealias WalletConnectServer = WalletConnectSwift.Server
public typealias WalletConnectRequest = WalletConnectSwift.Request
public typealias WalletConnectResponse = WalletConnectSwift.Response
public typealias WalletConnectSessionWalletInfo = WalletConnectSwift.Session.WalletInfo
public typealias WalletConnectSessionConnectionCompletionHandler = (WalletConnectSessionWalletInfo) -> Void
public typealias WalletConnectTopic = String
