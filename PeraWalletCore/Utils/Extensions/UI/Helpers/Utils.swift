...
public func img(_ named: String, isTemplate: Bool) -> UIImage? {
    let image: UIImage?
...
public let verticalScale = UIScreen.main.bounds.height / 812.0 > 1.0 ? 1.0 : UIScreen.main.bounds.height / 812.0
public let horizontalScale = UIScreen.main.bounds.width / 375.0 > 1.0 ? 1.0 : UIScreen.main.bounds.width / 375.0 

public func runIfDebug(_ closure: () -> Void) {
    #if DEBUG
    closure()
    #endif
}

public func runIfRelease(_ closure: () -> Void) {
    #if !DEBUG
    closure()
    #endif
}
