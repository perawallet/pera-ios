...
public final class WCTransaction: Codable {
    public private(set) var unparsedTransactionDetail: Data? // Transaction that is not parsed for msgpack, needs to be used for signing
    public private(set) var transactionDetail: WCTransactionDetail?
    public let signers: [String]?
    public let multisigMetadata: WCMultisigMetadata?
    public let message: String?
    public let authAddress: String?

    /// ID is used to separate the transactions that contains exactly the same elements.
    public let id = UUID()
    public private(set) var requestedSigner = WCTransactionRequestedSigner()
...
extension WCTransaction {
    ...
    public var hasValidAuthAddressForSigner: Bool {
        switch signer() {
    ...
    public var hasValidSignerAddress: Bool {
        switch signer() {
        case let .current(address):
    ...
    public func isInTheSameNetwork(with params: TransactionParams) -> Bool {
        return
            transactionDetail?.genesisId == params.genesisId &&
            transactionDetail?.genesisHashData == params.genesisHashData
    }

    public func isFutureTransaction(with params: TransactionParams) -> Bool {
...
extension WCTransaction: Equatable {
    public static func == (lhs: WCTransaction, rhs: WCTransaction) -> Bool {
