...
//   WCTransactionRequestValidatedEvent.swift

import Foundation
import MacaroonVendors

public struct WCTransactionRequestValidatedEvent: ALGAnalyticsEvent {
    ...
    fileprivate init(
        ...
        self.name = .wcTransactionRequestValidated
        self.metadata = [
...
extension AnalyticsEvent where Self == WCTransactionRequestValidatedEvent {
    public static func wcTransactionRequestValidated(
        transactionRequest: WalletConnectRequest
    ) -> Self {
        return WCTransactionRequestValidatedEvent(
