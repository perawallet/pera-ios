...
final class InboxService: InboxServiceable, NetworkConfigureable {
    ...
    func acceptAccountImportRequest(jointAccountAddress: String) async throws(ActionError) {
        ...
        do {
            try await accountService.createJointAccount(participants: jointAccountImportData.participantAddresses, threshold: jointAccountImportData.threshold, name: jointAccountAddress.shortAddressDisplay)
        } catch {
